...
npm ci
                # install neon CLI
npx -y neonctl@2.15.0 --version >/dev/null
                ...
                npx -y neonctl@2.15.0 branches create --project-id "$NEON_PROJECT_ID" --parent "$NEON_PARENT_BRANCH_ID" --name "$EPHEMERAL_BRANCH_NAME" --compute --output json > neon_branch.json --api-key "$NEON_API_KEY"
...
# exporting here overrides the global DATABASE_URL env var for this shell step onl
       ...
       export DATABASE_URL="$(
  npx -y neonctl@2.15.0 connection-string "$EPHEMERAL_BRANCH_NAME" \
    ...
    post {
      ...
      if [ -n "$EPHEMERAL_BRANCH_NAME" ]; then
       npx -y neonctl@2.15.0 branches delete "$EPHEMERAL_BRANCH_NAME" --project-id "$NEON_PROJECT_ID" --api-key "$NEON_API_KEY"
      fi
    '''
            }
        success {
